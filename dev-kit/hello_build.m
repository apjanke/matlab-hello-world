...
reporoot = hello.globals.distroot;
origDir = pwd;
RAII.cd = onCleanup(@() cd(origDir));
cd(reporoot)

fprintf('Building matlab-hello-world...\n')

cfgFile = fullfile(reporoot, '.mlproject.json');
config = jsondecode(hello.internal.util.readtext(cfgFile));
...
fprintf('matlab-hello-world built.\n')
