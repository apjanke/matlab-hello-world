...
% that's done in hello.Settings.discover. It has to be that way so the package
% settings state can handle a `clear classes` gracefully.

% Do not re-initialize if already initialized
if hello.internal.utils.getpackageappdata('initialized')
  return
end

% Don't depend on globals, to avoid circular dependency
%distroot = hello.globals.distroot;
...
hello.internal.utils.setpackageappdata('initialized', true);
