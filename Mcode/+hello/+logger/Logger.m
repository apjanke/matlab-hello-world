classdef Logger
  ...
  % arguments.
  %    hello.logger.info(format, varargin)
  ...
  % message.
  %    hello.logger.warn(exception, format, varargin)
  %
  % See also:
  % hello.logger.error
  % hello.logger.warn
  ...
  % Examples:
  %
  % log = hello.logger.Logger.getLogger('foo.bar.FooBar');
  ...
  methods (Static)
    function out = getLogger(identifier)
      ...
      out = hello.logger.Logger(jLogger);
    end
  end
