classdef Log4jConfigurator
    ...
    % Examples:
    %
    % hello.logger.Log4jConfigurator.configureBasicConsoleLogging
    %
    % hello.logger.Log4jConfigurator.setLevels({'root','DEBUG'});
    %
    % hello.logger.Log4jConfigurator.setLevels({
    %     'root'    'INFO'
    %     'net.apjanke.logger.swing'  'DEBUG'
    %     });
    %
    % hello.logger.Log4jConfigurator.prettyPrintLogConfiguration
    %
    % % Display fully-qualified class/category names in the log output:
    % hello.logger.Log4jConfigurator.setRootAppenderPattern(...
    %    ['%d{HH:mm:ss.SSS} %p %c - %m' sprintf('\n')]);
    %
    % % Bring up the configuration GUI
    % hello.logger.Log4jConfigurator.showGui
    ...
    methods (Static)
        ...
        function setLevels(levels)
            ...
            %
            % hello.logger.Log4jConfigurator.setLevels(levels)
            %
            ...
            for i = 1:size(levels, 1)
                ...
                level = hello.logger.Log4jConfigurator.getLog4jLevel(levelName);
